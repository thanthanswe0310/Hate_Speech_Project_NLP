...
# import sys
# sys.path.append(PATH_TO_HSLE + '/src')
...
# Constants
EXT = 'csv'
OUTPUT_PATH = '../clean-data/aggregated/{}'.format(datetime_suffix())
...
def run(comment_files):
    ...
    for comment_file in comment_files:
        ...
        # now has topics_found, page_group_name
...
def load_df(comment_file, date_from, date_to):
    ...
    comment_df = comment_df.loc[np.logical_and(
        comment_df.datetime>date_from, comment_df.datetime<date_to), :]
    ...
    post_df = pd.read_csv(post_file, usecols=['page_group_name','post_url', 'lex_found'])
    # join
...
# def hs_ratio(lex_datetime_df):
#     lex_datetime_df['date'] = [
#         pd.datetime(d.year, d.month, d.day)
#         for d in lex_datetime_df.datetime]
#     # split into no lex and with lex
#     no_lex = lex_datetime_df.loc[lex_datetime_df.lex_found.isna(),:]
#     w_lex = lex_datetime_df.loc[~lex_datetime_df.lex_found.isna(),:]
#     # calculate counts
...
#         'HS%': wlex / (nlex + wlex)
#     })
#     save_dataframe(ratiodf, 'ratio', index=True)

def page_hs_ratio(comment_df):
    comment_df['has_lex'] = ~comment_df.lex_found.isna()
    lex_count = comment_df.groupby(['page_group_name','date'])['has_lex'].sum()
    total = comment_df.groupby(['page_group_name','date'])['page_group_name'].count()
    total.name = 'total'
    page_hs_ratio_df = pd.DataFrame([lex_count, total]).T
    page_hs_ratio_df['ratio'] = page_hs_ratio_df.has_lex / page_hs_ratio_df.total
    save_dataframe(page_hs_ratio_df, 'page-hs-ratio', index=True)
